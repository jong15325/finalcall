...
- Atlassian MCP는 전역 설정에서 enabled와 required를 모두 true로 유지한다. 새 세션 초기화 또는 작업 중
  연결/OAuth가 끊기면 즉시 사용자에게 알리고, 비대화형 상태 확인·복구는 자동 수행한다. 브라우저 OAuth 승인처럼
  사용자 입력이 필요하면 정확한 CLI 명령과 승인 필요성을 즉시 안내한다. Jira 미러는 복구 전후에도 생략하지 않는다.
