...
Spring Boot 대규모 트래픽 스켈레톤 프로젝트의 Codex 지침이다.
...
이 파일은 변수, 전역 원칙, Codex 행동 규약을 담는다.
...
**패키지 레이아웃 = feature-first**(EPIC-RESTRUCTURE, 2026-07-25 게이트2). 최상위를 기술 계층(구 `api`/`domain`)이 아니라 **도메인(feature)** 으로 분할하되, 업무 도메인 feature들은 **`com.finalcall.domain` 그룹 아래**로 묶고 각 feature 내부에 `controller/service/repository/entity/dto` 계층 하위패키지를 둔다 — 패키지는 `com.finalcall.domain.<feature>.<layer>`(예: `com.finalcall.domain.member.service.MemberService`, `com.finalcall.domain.member.entity.User`). 횡단 인프라·공용 커널은 feature가 아니므로 **`domain` 그룹 밖** `com.finalcall.common`·`com.finalcall.infra`에 제자리로 남는다(domain 밑으로 넣지 않는다). 상세 규약·목표 레이아웃·ArchUnit 규칙 스펙 = `docs/common/proposals/layer-restructure-proposal-v0.1.md`(파일명은 v0.1이나 내용은 v0.4 DECIDED). 재구성은 전 도메인 완료됐다(EPIC-RESTRUCTURE, `com.finalcall.api.*` 소멸 · 최상위 = `common`·`domain`·`infra`·`support`). 이어 **EPIC-CONVENTION-V2**가 feature *내부* 어휘·위치 규약을 확정했다(ErrorCode 중앙화·Properties 소비범위 분리·DTO 어휘 축약 — 섹션 5·proposal §9.7~§9.10).
...
- **git commit / push 는 사용자가 직접 한다. Codex 는 커밋·푸시하지 않는다.
...
```
PACKAGING        = feature-first                   # 업무 도메인 feature=com.finalcall.domain.<feature>.<layer> (domain 그룹 아래로 묶음)
FEATURE_LAYERS   = controller > service > repository > entity   # feature 내부 계층·의존방향(왼→오 단방향). dto는 controller/service가 참조, entity에만 의존
KERNEL_PACKAGES  = common, infra                   # 횡단 커널(feature 아님·제자리, domain 그룹 밖). 어떤 feature도 이 둘을 역으로 의존받지 않음
SAMPLE_FEATURE   = sample                          # 새 경로: com.finalcall.domain.sample.{controller,service,dto,...}
...
- **의존 방향(feature-first, EPIC-RESTRUCTURE)**: 세 축을 ArchUnit(`SliceArchitectureTest`·`ConventionArchitectureTest`)이 기계 강제한다.
  (1) **슬라이스 내부 계층방향** — 한 feature 안에서 `controller → service → repository → entity`(+ `dto`) 단방향. entity/repository는 controller/service를 역참조 금지.
  (2) **커널 무의존** — `common`·`infra`는 `domain` 그룹 밖 커널로, 어떤 feature도 의존하지 않는다(`common`은 프레임워크 최소 의존·가능한 순수 Java, JPA/Redis 등은 feature/`infra`에만).
  (3) **슬라이스 비순환** — feature 간 순환 참조 금지.
  구조 규칙 스펙 = proposal §10 (a)(b)(c). V2 어휘·위치 규칙 (e)(f)(g)는 `ConventionArchitectureTest`가 강제한다(섹션 5).
...
- **물리 배치(feature-first, EPIC-RESTRUCTURE)**: 각 클래스는 `com.finalcall.domain.<feature>.<layer>`에 둔다 — Controller→`controller`, Request/Response 등 표현 DTO→`dto`, Service·도메인 VO→`service`, Repository(+Custom/Impl)→`repository`, Entity·귀속 enum→`entity`. **도메인 예외(`*Exception`)만 feature 루트**에 남는다. 공용 커널(`common`·`infra`)은 `domain` 그룹 밖 제자리. 상세 분류표·경계 사례는 proposal §9. 아래 네이밍·설계 규칙은 배치와 무관하게 그대로 적용된다.
...
- **DTO**: Java `record`, Response 는 `@Builder` + `static from(Entity)`, 네이밍 `<도메인><목적>Request/Response`(Dto 접미사 금지).
- **DTO 어휘(V2 축1, proposal §9.7)**: 웹 표현 DTO 허용 접미사 = `Request`·`Response`·공용 `CursorResponse<T>` 3종뿐. `*View`·`*Detail`·`*Slice` 접미사 **폐지**(중첩 read 모델은 상위 `*Response` 내부 record로, 커서 목록은 `CursorResponse<T>`로). 서비스 계약 DTO `*Command`·`*Result`는 **bid·settlement feature에서만** 허용(그 외 feature는 서비스가 웹 DTO를 직접 수령·반환), 배치는 해당 feature `dto/`. `ConventionArchitectureTest` (e-1)(e-2)가 강제.
- **도메인 ErrorCode(V2 축2, proposal §9.8)**: 공통 `ErrorCode` 인터페이스를 구현한 도메인별 enum, 네이밍 `{DOMAIN}_{3자리}`. **위치 = `com.finalcall.common.exception`으로 중앙화**(v1의 "feature 루트" 번복 — 도메인별 enum 분리는 유지, 병합 금지). 예외: `infra.security.GatewayErrorCode`는 엣지 인프라 코드로 infra 잔류. `ConventionArchitectureTest` (f)가 강제.
- **Properties(V2 축3, proposal §9.9)**: `@ConfigurationProperties`는 소비범위로 배치 — 단일 feature 전용→`com.finalcall.domain.<feature>.config`, 인프라 어댑터 종속 cross-cutting→`infra/config`, 계층중립 cross-cutting→`common`. **feature 루트 직속 금지**. `ConventionArchitectureTest` (g)가 강제.
...
- Codex 는 단계 완료 시 이 템플릿으로 채운 커밋 메시지를 제안한다(실행은 사용자).
...
### Codex 연계
- 커밋·푸시는 사용자가 직접 실행한다.
- Codex 는 단계·작업 완료 시 위 컨벤션을 따른 커밋 메시지를 제안한다(여러 변경이면 나눠서).
...
- **적용 의무**: 코드 작성·커밋 전 `./gradlew :backend:spotlessApply` 실행 후 checkstyle 통과를 확인한다.
  ...
  새 세션·전 도메인 동일 적용한다(Codex 킥오프도 이 절을 따른다).
...
- 총괄과 모든 서브에이전트의 모델은 **gpt-5.6-sol로만 고정**한다.
- 전역 기본값, 프로젝트 기본값, 역할별 설정을 모두 동일하게 유지한다.
- 에이전트 생성 시 다른 모델 override를 지정하지 않는다. 세션이 바뀌어도 이 규칙을 적용한다.
- Jira 미러 전송은 **Jira Cloud REST API**를 사용한다. 메인세션은 새 세션 초기화 또는 작업 중
  REST API 연결·인증 상태를 확인하고, 실패하면 즉시 사용자에게 알린 뒤 비대화형 복구를 자동 수행한다.
  인증값은 환경변수로만 주입하고 출력·문서·커밋에 남기지 않는다. Jira 미러는 복구 전후에도 생략하지 않는다.

**우선순위**: 이 절부터 섹션 13까지(오케스트레이션 모드)는 섹션 1~7의 개발 규약 위에서 동작한다. **커밋·단계 진행에 관해 섹션 2·6과 충돌하면 이 절들이 우선한다**(섹션 2의 "한 단계씩"은 스켈레톤 스테이징 기준이며, 커밋은 작업 모드와 무관하게 섹션 13의 사용자 승인 절차를 따른다).
...
- **보안 층(경매 에픽부터 첫 적용)**: 파이프라인 골격은 불변이고 보안 층만 겹친다 — (1) 구현 중 커밋 보안 리뷰(warn-only) + 선택적 end-of-turn 리뷰, (2) reviewer가 확인소로 도메인 인가 최종 판정, (3) 에픽 완료 직전 온디맨드 `/security-review` 1회, (4) 사용자 push 후 원격 CI(정적분석·의존성 스캔), (5) 공통 위협모델 체크리스트(`.Codex/Codex-security-guidance.md`)를 플러그인·reviewer가 공유 참조. member/account에 소급하지 않는다.
...
| **디자인 게이트** | frontend-impl이 **새 화면·주요 UI** 구현 전 | 실제 프론트 dev-only `/__design/<scenario>` 워크벤치에서 **1회 경량 검증**(390·1280px 레이아웃·overflow·대비·실제 frontend 정본 재사용·production residue) 후 승인·조정한다. 목업 단계에서는 전체 보안·전체 회귀·정식 reviewer를 수행하지 않고 실제 운영 반영 후 Done 전에 수행한다. critical/major 발견 시에만 1회 수정 확인을 허용한다. 정적 HTML 목업은 역사 참고만 하며 **단순 수정은 자동** |
| **게이트3 (push + Done)** | 에픽 완료 시 | **push는 사용자가 직접 실행**(에이전트 불가, PreToolUse 훅이 차단). **Done 전이는 사용자 승인**. 커밋은 작업별 사용자 승인 후 실행 |

- **보안 리뷰는 게이트가 아니다.** 커밋 보안 리뷰는 warn-only(비차단)이며, 사용자 커밋 승인 요청 전에 결과를 함께 보고한다. 보안 리뷰 자체는 별도 승인 단계를 만들지 않는다. 상신이 필요한 보안 결정(스키마·계약·인가 모델 변경)은 기존 **게이트2**로 수렴한다.
...
- **파일 → Jira 단방향·전건 미러**. Jira Cloud(KAN)는 **사용자 전용 대시보드**이며, 전송 수단은 **Jira Cloud REST API**다. 모든 에픽과 하위 task를 Jira 이슈로 발급한다. 메인세션만 미러·패리티 대조를 위해 Jira를 읽고 쓰며, 서브에이전트는 Jira에 접근하지 않는다.
- **트리거**: **메인세션만** 에픽·task 생성과 상태 전이 **때마다 즉시** 반영한다. **비차단**은 미러 실패 시 파일 작업을 멈추지 않는다는 뜻이며 — **실패 허용이지 생략 허용이 아니다**(가시성 도구, 게이트 아님).
- **제목·설명 템플릿**: Jira task summary는 반드시 파일의 `id`와 `title`을 조합한 **`FC-NNN · <title>`**로 쓴다. 에픽 설명은 배경 → 목표 → 범위 → 하위 작업(파일 티켓 ID) → 완료 기준 → 게이트 → 정본 경로, task 설명은 목표 → DoD → 의존 → 게이트 → 정본 경로 순서로 작성한다.
...
- **드리프트 가드레일**: (계층①·자동) 템플릿을 제외한 에픽·task의 `jira_key`가 비어 있으면 상태와 무관하게 "Jira 미생성" 드리프트다. `git commit` 전 `.codex/hooks/check-mirror-drift.js`가 전건의 빈 key와 task의 `FC-NNN` ID·파일명·title 형식을 탐지해 **경고**한다(warn-only, 커밋 비차단). (계층②·수동) 파일 보드 전건과 Jira의 key·summary·상태·에픽 귀속·관계 링크 불일치는 총괄이 출근·퇴근 HANDOVER 패리티 단계에서 대조·보정한다.

## 섹션 13: 커밋·push 규약 (오케스트레이션 모드)

- **커밋 = 사용자 사전 승인**. 작업 완료 후 커밋하지 않은 상태에서 변경 파일과 주요 변경 내용을 보고하고 커밋 메시지를 제안한 뒤 승인을 요청한다. 사용자가 명시적으로 승인한 경우에만 해당 범위를 atomic commit한다.
- **push = 에픽 완료 시 사용자가 직접 실행**. 에이전트는 push 권한이 없다. **PreToolUse 훅은 `git push`를 차단**한다. `git commit`의 실행 여부는 위 사용자 승인 절차로 통제한다(게이트3 훅 · settings.json).
- **Done 전이 = 사용자 승인**(게이트3).
...
- **커밋 보안 리뷰 = warn-only(비차단)**. 발견을 재프롬프트할 뿐 커밋을 막지 않는다. 보안 리뷰 자체는 별도 승인 단계를 만들지 않으며, 결과는 커밋 승인 요청에 포함한다. `block-git-push`는 push만 차단하며 무간섭이다.
  - **배선 주의(재프롬프트↔review 전이 타이밍)**: 재프롬프트가 턴을 연장해 에이전트가 "구현 완료" 이후 추가 편집을 할 수 있다. 티켓 `review` 전이 시점과 이 후속 편집이 엇갈리지 않도록, 메인세션은 **후속 편집이 수렴한 뒤** review로 전이한다.
...
- **한도 폴백**: 사용량 한도 압박 시 자동 리뷰 층(커밋 시 보안 리뷰·end-of-turn)은 끄고 온디맨드 `/security-review`만 유지한다.
