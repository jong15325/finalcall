...
public interface AuctionRepositoryCustom {
     ...
     * (정본) 경매 전건을 {@code status} 로 그룹핑한 count 를 반환한다 — ES {@code terms} agg(status) 와 대조해 총량이
     * 같아도 상태 분포가 어긋나는 <b>상태전이 드리프트</b>를 잡는다. 색인기가 전 상태를 색인하므로(필터 없음) 전건 집계다.
     *
     * @return 상태 → 행 수(해당 상태 행이 없으면 키 부재)
     */
    Map<AuctionStatus, Long> countGroupByStatus();

    /**
     * 화해 histogram 대조용 가격 버킷별 count(EPIC-SEARCH · search-spec §12.5). 가격({@code startPrice} — ES 색인
     * {@code price} 와 정합)을 {@code floor(startPrice/size)*size} 하한 키로 그룹핑한 count 를 반환한다. ES
