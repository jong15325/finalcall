...
public interface ShopRepository extends JpaRepository<Shop, Long>, ShopRepositoryCustom {
    ...
    @Query("SELECT new com.finalcall.domain.shop.ShopPurchaseContext("
     ...
     */
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT new com.finalcall.domain.shop.ShopExpireContext("
    ...
    @Query("SELECT s.id FROM Shop s "
        + "WHERE s.status = com.finalcall.domain.shop.ShopStatus.ACTIVE "
     ...
     */
    @Modifying
    @Query("UPDATE Shop s SET s.status = com.finalcall.domain.shop.ShopStatus.SOLD "
        + "WHERE s.id = :id "
        + "AND s.status = com.finalcall.domain.shop.ShopStatus.ACTIVE "
     ...
     */
    @Modifying
    @Query("UPDATE Shop s SET s.status = com.finalcall.domain.shop.ShopStatus.CANCELLED "
        + "WHERE s.id = :id AND s.status = com.finalcall.domain.shop.ShopStatus.ACTIVE")
     ...
     */
    @Modifying
    @Query("UPDATE Shop s SET s.status = com.finalcall.domain.shop.ShopStatus.EXPIRED "
        + "WHERE s.id = :id "
        + "AND s.status = com.finalcall.domain.shop.ShopStatus.ACTIVE "
        + "AND s.endAt IS NOT NULL AND s.endAt <= :now")
