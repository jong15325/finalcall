 ...
 *
 * ★★ **버튼 AA 는 프리셋 교체로 해결됐다**(FC-057, 사용자 판정 2026-07-19 — 선택지 ①).
 *    템플릿 기본 프리셋(blue)에서는 `solid` 정지 3.56 · hover 2.96 이었고, `default`·`plain`
 *    도 hover 가 primary 램프를 지나 3.56/2.96 이라 **세 variant 전부** 미달이었다 —
 *    variant 교체로는 풀리지 않았다. WCAG 1.4.3 큰 텍스트 예외도 **적용되지 않는다**(실측:
 *    버튼 활자 xs 12 / sm 14 / md 14(body 상속) / lg 16px, 전부 `.button` 의 `font-bold` →
 *    임계값 18.66px bold 미달).
 *    → `configs/theme.config.ts` 의 `themeSchema` 를 템플릿 프리셋 **`dark`**(`--primary`
 *      #18181B)로 골랐다. **토큰을 덮은 게 아니라 템플릿이 제공하는 선택지를 고른 것**이다.
 *      결과: solid 정지 17.72 / hover 14.89(라이트), 17.74 / 14.33(다크) — **전 상태 통과**.
 *
 * ★ 다크 모드에서는 프리셋이 `--primary: #FFFFFF`·`--neutral: #111827` 을 주므로
 *   **주 CTA 가 흰 채움 + 어두운 글자로 반전된다**(17.74). 버그가 아니라 프리셋 설계다.
 */
