...
export const apexLineChartDefaultOption: ApexOptions = {
    ...
    legend: {
        ...
        // apexcharts 타입상 opts 는 optional 이고 dataPointIndex 를 선언하지 않는다.
        // 템플릿 원본은 무검사로 접근해 타입체크에서 걸린다 — 가드 + 좁은 캐스트로 해소한다.
        tooltipHoverFormatter: function (val, opts) {
            if (!opts) return val
            const { w, seriesIndex } = opts
            const dataPointIndex = (
                opts as unknown as { dataPointIndex?: number }
            ).dataPointIndex
            if (dataPointIndex === undefined) return val
            return val + ' - ' + w.globals.series[seriesIndex][dataPointIndex] + ''
        },
    },
    xaxis: {
        categories: [],
