...
export interface ParticleStudyOption {
    id: string
    rank: number
    name: string
    principle: string
...
export interface ParticleStudyFixture<
    TOption extends ParticleStudyOption,
> extends WorkbenchFixture {
    options: readonly TOption[]
}

export default function ElementParticleStudyScenario<
    TOption extends ParticleStudyOption,
>({
    fixture,
    element,
    ...
    fixture: ParticleStudyFixture<TOption>
    element: 'fire' | 'water'
    title: string
    region: string
    summary: string
    renderPreview: (option: TOption) => React.ReactNode
}) {
    const [searchParams] = useSearchParams()
    const requested = searchParams.get('variant')
    const selected =
        fixture.options.find(({ id }) => id === requested) ?? fixture.options[0]
    const scenarioId = `${element}-particle-studies`

    return (
        <div
            data-testid={`${element}-particle-scenario`}
        ...
        >
            ...
            <section
                aria-labelledby={`${element}-comparison-title`}
                className="w-full min-w-0 max-w-full"
            >
                <div className="flex flex-wrap items-end justify-between gap-3">
                    <div>
                        <h2
                            id={`${element}-comparison-title`}
                ...
                <div className="mt-4 grid w-full min-w-0 max-w-full gap-4 sm:grid-cols-2">
                    {fixture.options.map((option) => {
                        ...
                        return (
                            <article
                                key={option.id}
                                data-particle-variant={option.id}
                            ...
                            >
                                ...
                                <Link
                                    to={`/__design/${scenarioId}?variant=${option.id}`}
