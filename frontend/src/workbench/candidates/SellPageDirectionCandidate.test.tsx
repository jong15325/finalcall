...
} from './SellPageDirectionCandidate'

describe('SellPageDirectionCandidate', () => {
    it.each(SELL_DIRECTION_VARIANTS)(
        '%s 안은 필수 판매 정보 전체를 노출한다',
        (variant) => {
            ...
            expect(
                container.querySelector(`[data-sell-direction="${variant}"]`),
            ).toBeInTheDocument()
    ...
    it('기간과 판매 방식을 실제로 비교할 수 있다', async () => {
        ...
        expect(
            screen.getByText('판매 기한은 서버가 자동 설정합니다.'),
        ).toBeVisible()
    ...
    it('세 방향은 서로 다른 정보 구조를 사용한다', () => {
        ...
        expect(
            canvas.container.querySelector('[data-direction-region="summary"]'),
        ).not.toBeInTheDocument()
        expect(
            canvas.container.querySelector('[data-direction-region="steps"]'),
        ).not.toBeInTheDocument()
        expect(
        ...
        expect(
            screen.getByRole('region', { name: '모바일 판매 등록' }),
        ).toBeVisible()
        expect(
            canvas.container.querySelector('[data-direction-region="item"]'),
        ).toHaveClass('shop-cardinfo')
        expect(
            canvas.container
        ...
        expect(
            canvas.container.querySelector('.ci-panel .skill-list'),
        ).toBeInTheDocument()
        canvas.unmount()

        const guided = render(<SellPageDirectionCandidate variant="guided" />)
        expect(
            guided.container.querySelector('[data-direction-region="steps"]'),
        ).toBeInTheDocument()
    })
})
