/**
 * 아이템 코드 사전 — **정본은 계약 v1.10 §3.3.1**(게이트2 FC-044 승인).
 *
 ...
 *
 * `art` 는 카드 아트 파일명(`docs/game_ui/.../{element}/{art}.png`)이다. **마법 2종은 `magic.png`
 * 한 장을 공유한다**(2:1) — 원본 아트에 특수/일반 구분이 없다.
 */
const KINDS_BY_SUB_GROUP: Record<number, readonly { code: number; label: string; art: string }[]> =
  {
    1: [
...
export function subGroupLabelOf(subGroup: number): string {
  return SUB_GROUPS.find((entry) => entry.code === subGroup)?.label ?? `대분류 ${subGroup}`;
}

/** 대분류에 속한 종류 목록. 미등록 대분류는 빈 배열 — 필터가 선택지를 지어내지 않는다. */
export function kindsOf(subGroup: number): readonly { code: number; label: string }[] {
...
export function kindLabelOf(subGroup: number, kind: number): string {
  return kindsOf(subGroup).find((entry) => entry.code === kind)?.label ?? `종류 ${kind}`;
}

/** 아트 파일명 조각. 미등록 조합은 null — 소비처는 플레이스홀더로 폴백한다. */
export function kindArtSlugOf(subGroup: number, kind: number): string | null {
  return KINDS_BY_SUB_GROUP[subGroup]?.find((entry) => entry.code === kind)?.art ?? null;
}
