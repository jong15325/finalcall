 ...
 * 그럴듯하다. 그래서 이 모듈의 **단위 테스트가 유일한 방어선**이다(`itemArt.test.ts`).
 *
 * 그 밖의 규칙:
...
/**
 * 원본 픽셀 크기(24bpp 실측, design-system [2.7] 주).
...
export interface ItemArt {
  src: string;
  /** 표시 크기(정수배). 비정수 확대는 픽셀아트를 뭉갠다 — 확대는 정수배만이다([5.3]). */
  width: number;
  height: number;
}

/** 아트 파생에 필요한 최소 입력. `ItemBlock` 전체를 요구하지 않아 테스트가 가볍다. */
...
/** `ItemBlock`(계약 §3.3)에서 필요한 축만 뽑는다. */
export function toArtInput(item: ItemBlock): ItemArtInput {
  return {
...
export function itemArtSrc(input: ItemArtInput, size: ArtSize): string | null {
  ...
  if (!Number.isInteger(level) || level < ART_LEVEL_MIN || level > ART_LEVEL_MAX) return null;
 ...
 */
export function itemArt(input: ItemArtInput, size: ArtSize, scale: number): ItemArt | null {
  const src = itemArtSrc(input, size);
  if (!src) return null;

  const base = ART_BASE_SIZE[size];
  const factor = Math.max(1, Math.trunc(scale));
  return { src, width: base.width * factor, height: base.height * factor };
}
