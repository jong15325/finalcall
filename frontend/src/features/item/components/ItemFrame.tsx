 ...
 *
 * ★ **골드포스·STANDARD(블랙)·스킬 마크만 실제 렌더**한다. 골드포스는 `gold-Photoroom.png` 프레임
 ...
 *   렌더러를 두지 않는다(도달 불가 = 죽은 코드 금지) — `resolveFrameType` 이 STANDARD 로 폴백한다.
...
function ItemFrame({
    ...
    const isGoldforce = resolveFrameType(visual, resolvedNow) === 'GOLDFORCE'
    const days = isGoldforce
