 ...
 *
 * ══════════════════════════════════════════════════════════════════════════════
 * ★★ **스킬 이름이 실제로 표시되는 유일한 화면이다.**
 * ══════════════════════════════════════════════════════════════════════════════
 * 경매 상세(`AuctionItemView`)는 스킬 **코드(정수)만** 실어 `스킬 #{code}` 중립 표기가 계약
 * 준수(§2.5)지만, 인스턴스 상세는 `{skillCode, name}` 을 주므로 **실제 이름**을 낸다. 슬롯 해소는
 * 보존 `resolveSkillSlots`/`skillLabelOf` 를 그대로 재사용한다 — 이름이 있으면 이름, 없으면 중립
 * 코드로 흐르는 분기가 그 lib 한 곳에 있어 두 맥락의 일관성이 보장된다(m-5 슬롯 오표기도 방지).
 *
                ...
                <section className="rounded-2xl border border-line bg-surface p-6">
                    ...
                    {canRegister && (
                        <div className="mt-6 flex flex-wrap gap-2">
                            <Link
                                to={paths.sell}
                                className="inline-flex items-center gap-1.5 rounded-lg bg-navy px-4 py-2.5 text-sm font-bold text-white hover:bg-navy-800"
                            >
