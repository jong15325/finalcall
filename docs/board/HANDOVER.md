...
8. **★ 정정(2026-07-20) — 종전 8번은 "파일 이동 `git mv` 금지(C-075)"였는데 규칙이 거꾸로 적혀 있었다.**
   **C-075는 그 금지를 *삭제한* 결정이다.** 원 금지(`[9.4]`)는 *"`mv`가 내용을 읽어서 옮긴다"*는
   **추론** 위에 세워졌으나, 실측 결과 같은 마운트 안 `mv`는 `rename(2)`라 **inode가 보존되고
   내용을 읽지 않는다**는 것이 확인돼 반증됐다(결정로그: *"`[9.4]`는 그 추론 위에 세운 조항이었고
   C-075가 삭제했다"*). **파일 이동은 자유다.** 근거 `docs/common/rules.md` C-075 ·
   `docs/consulting/decision-log.md` C-059(SUPERSEDED → C-075).
9. 통신은 파일로. Jira 미러는 상태 전이마다 즉시(메모리 `jira-mirror-discipline`).
10. **티켓은 계약의 사본이 아니라 요약이다.** FC-064 티켓이 `BID_002`를 "하한 미달"로 잘못 적었고
   (실제는 `BID_001`), frontend-impl이 **계약을 정본으로 삼아** 올바르게 구현한 뒤 오기를 보고했다.
   **총괄이 티켓에 계약 세부를 옮겨 적을 때 그 자체가 새 오류원이 된다** — 코드를 옮기지 말고
   **출처를 가리켜라.**
