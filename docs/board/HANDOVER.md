...
# 🚨 이 파일 대부분은 2026-07-20자로 낡았다 — 먼저 읽어라

**사용자가 디자인 전면 변경을 결정했다(네 번째 방향 전환).** 아래 본문이 서술하는
**EPIC-FE-ECME는 `superseded`**이고, 현재 에픽은 **`EPIC-FE-REBUILD`(KAN-73, `blocked`)**다.

> "디자인 전면 변경할 예정이야, 디자인 목업을 준비해올게. 우리가 하던 디자인은 전부 폐기 진행하고
> 프론트도 작업한 것도 폐기 다시 할 예정"

**지금 상태**: **사용자 목업 도착 대기.** 목업이 없는 상태에서 **아무것도 지우지 않았다** —
무엇으로 대체되는지 모른 채 파괴하는 것이기 때문이다. 코드는 그대로 있고 보드만 전이했다.

**따라서 아래 본문에서 아직 유효한 것 / 죽은 것**:
| 절 | 지위 |
|---|---|
| "환경 기동 — 함정 3개" | **유효**(인프라·시드·크로마키). 단 함정 B(테마 프리셋)는 템플릿과 함께 죽는다 |
| "현재 상태 — 프론트 = Ecme 기반" | **전부 죽음**(스택 표·디자인 방침 3단 정정·완료 티켓 표) |
| "다음 수" | **전부 죽음** |
| "미결" · "게이트2 후보" | **계약 관련은 유효**, 디자인 관련은 죽음. 최신 계약 분석은 `tickets/FC-064.md` |
| "유효한 자산" | **유효** — §5.12는 원본 자산 실측이라 **선제 폐기하지 않는다** |
| "이 세션의 교훈" | **유효**(특히 3번 과폐기 금지 · 6번 모바일 우선 · 8번 C-075 정정) |

**이어받는 법(갱신)**: `epics/EPIC-FE-REBUILD.md`를 먼저 읽어라 — **보존 45파일 / 폐기 318파일
목록**과 **목업 도착 시 확인할 것 4가지**, **승계된 발견**이 거기 있다.

**★ 이 HANDOVER 전면 갱신은 사용자 지시 대기 중이다**(총괄이 임의 교체하지 않는다).
목업이 도착해 새 에픽이 `doing`이 될 때 갱신하는 것이 자연스럽다.
