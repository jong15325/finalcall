...
- **D-090(092 발번, 최신·최우선)** — 저장소 파일의 목록·내용·형상·편집 위치 판단은 **호스트
  도구(Read·Grep·Glob·Edit)로만**. bash 마운트 뷰 판단 금지(존재·부재·형상·위치·수량 전부).
  bash는 git·빌드·테스트 전용. **S-003을 대체**한다 — S-003의 "긍정 근거는 자기 증명적이라
  교차검증 불요" 비대칭은 폐기(stale은 긍정 형태로도 서빙된다). D-043의 교차검증 조항도 대체.
- D-086(080 발번) — 부재·수량을 근거로 삼을 때 탐색 방법 1구 명시. 유효(D-090과 결합).
