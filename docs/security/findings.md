...
| SEC-015 | Minor | 게이트웨이 공유비밀을 `String.equals`로 비교 — 비상수시간 | OPEN | 구현 |
...
## SEC-015. 게이트웨이 공유비밀을 `String.equals`로 비교한다 — 비상수시간

심각도: Minor · 상태: OPEN · 2026-07-16 (025 보안 구현 리뷰 산출)
관련: `backend/src/main/java/com/finalcall/infra/security/GatewayAccessFilter.java:57`,
       D-068, B-027, SEC-005

조건
- 직접접근 차단이 공유비밀을 이렇게 대조한다(호스트 Read 원문):

  > `if (!StringUtils.hasText(token) || !properties.secret().equals(token)) {`

- **`String.equals`는 첫 불일치 바이트에서 즉시 반환한다**(단축 평가). 응답 시간이 **일치한
  접두사 길이에 비례**하므로, 공격자가 서비스에 직접 요청을 반복하며 응답 시간을 재면
  이론상 공유비밀을 **바이트 단위로 좁혀 나갈 수 있다**(원격 타이밍 사이드채널).
- **그리고 이 경로에는 rate limit이 없다.** rate limit은 게이트웨이 라우트 필터인데,
  이 공격은 **게이트웨이를 우회해 서비스를 직접 때리는 것**이 전제다. 시도 횟수 제한이 없다.

기대 vs 실제
- 기대: 시크릿 대조는 **상수시간**으로 한다(`MessageDigest.isEqual` 등).
- 실제: 단축 평가 비교라 시간이 내용에 의존한다.

조치(권고)
- `MessageDigest.isEqual(secret.getBytes(UTF_8), token.getBytes(UTF_8))`로 교체. **한 줄이다.**
  길이 차이는 새지만(조기 반환) **내용은 안 샌다** — 고엔트로피 시크릿에서 길이 노출은 무해하다.
- 배정: 백엔드. **비차단** — 아래 심각도 근거 참조.

심각도를 Minor로 둔 근거 (과장하지 않는다)
- **원격 타이밍 공격은 실용성이 낮다.** 네트워크 지터가 `String.equals` 한 바이트 차이(ns 단위)를
  압도한다. LAN 수준에서 실증된 사례는 있으나(Crosby et al. — μs 해상도) 다수 샘플과 안정적
  네트워크가 전제다.
- **시크릿이 고엔트로피다**(운영은 `GATEWAY_INTERNAL_SECRET` 주입, `@NotBlank` fail-fast).
  무차별 대입은 애초에 불가하고, 타이밍이 유일한 현실적 벡터인데 그마저 비싸다.
- **뚫려도 얻는 것이 「게이트웨이 우회」다** — 인증(JWT)은 그 뒤에 따로 있다(D-065, 서버 단독
  신원 결정). **자금·인증 우회가 아니다.** 얻는 실익은 rate limit 회피다(= SEC-005 무력화).
- **→ 그래서 Minor다. 다만 처방이 한 줄이라 비용 대비 이득이 크다.**

왜 지금 잡혔나
- **025 리뷰 범위에 있었는데 SEC-005 검증에서 놓쳤다.** SEC-005는 "차단이 존재하는가"를 봤고
  (`GatewayAccessFilter`가 403을 내는가 — O), **"차단이 어떻게 비교하는가"는 안 봤다.**
  같은 파일 같은 행을 읽고도 질문이 달라서 안 보였다. **범위가 다르면 같은 코드도 다시 봐야 한다.**
