...
>
> **`_archive/` 이관 (2026-07-16, C-073 [3] 기획분 집행).** 원 경로 `docs/design/notes/handover-kickoff-prompt.md`.
> 위 폐기 주가 이미 달려 있어 `notes/`(휘발성 흔적)에도 `references/`(인용 가능)에도 자리가 없다.
> `_archive/`는 `rules [3.5]`로 **읽지 않고 인용하지 않는** 구역이다 — 이 문서의 실제 성격과 일치한다.
