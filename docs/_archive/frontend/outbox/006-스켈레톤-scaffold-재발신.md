상태: SUPERSEDED → frontend/outbox/007-스켈레톤-scaffold-U020토큰반영.md (2026-07-15). 사유: 디자인 U-020(design-system v0.2)이 색 토큰 **명칭**을 픽셀 실측으로 확정 — [범위] 8 토큰 매핑의 A안 전제가 무효(ux/017). CC 미착수 상태(cc-reports 비어 있음)라 생성 전 편입이 재작업 0이므로 교체한다.
경위: 2026-07-15 프론트 repo 선생성 검토로 일시 SUPERSEDED 표기 → 사용자 철회로 SENT 원복(062 안건2 유지) → 이후 U-020 반영으로 007 교체. repo 후생성 전제는 007에서도 불변.
...
참조: `docs/frontend-planning/skeleton-plan.md` v0.2(선행 기준선 — 범위 [2]·IA 셸 [3]·토큰 방침 [4]·계약 정합 [5]·DoD [7]), `docs/api-contract.md` **v1.4**(계약 정본), `docs/frontend-planning/screen-spec.md` v0.3(IA·라우트), `docs/ux/design-system.md`(토큰 정본), 프론트 `CLAUDE.md` [3]~[6], F-002(구성), 062(레인 순서·repo 후생성).
...
8. **디자인 토큰(skeleton-plan [4])**: 정본은 `docs/ux/design-system.md` — **토큰명 1:1로 옮기되 값을 복제하지 않는다.** 테마 무관(`primary`·`accent`·semantic·`element` 4색) = Tailwind `theme.extend.colors` 정적값. 테마 의존(`bg`·`surface`·`surface-raised`·`border`·`text`·`text-muted`) = CSS 변수 + `[data-theme="dark"]` 오버라이드, Tailwind는 `var()` 참조. 색값은 **잠정 A안**(비주얼 미확정 — 확정 시 값만 교체되도록 토큰명·구조 고정). **등급(grade) 토큰 만들지 마라**(D-073).
...
- **비주얼 확정**(색값·타이포 최종), **실시간 채널**(SSE/WS — 폴링 전제 F-001, 구독 계층 두지 마라), **홈 통합 피드 표시 규칙**(탭 분리 잠정).
...
정본은 **파일**이다. 채팅 요약은 선택.
- 저장 경로: **`docs/frontend/notes/cc-reports/006-skeleton-scaffold.md`**
- 내용: 결과 요약 · 산출물(파일 트리 요지) · DoD 항목별 충족/미충족 · 계약·기획 공백이나 판단 보류 지점 · 커밋 메시지 제안.
