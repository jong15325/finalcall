...
IA 기준선 위상(D-077 → D-079 정정): 라우트↔엔드포인트 정보구조(IA)의 기준선은 frontend-planning/screen-spec.md가 소유한다. D-079로 프론트 기획(PF)이 기획(P)에 재통합되어, 이 산출물의 소유·조율 주체는 이제 기획(P)이다(frontend-planning/ 존치·P 이관). 본 노트는 그 IA에 정합하는 구현(F) 관점 분해이며, 어긋나면 screen-spec를 따르고 기획(P) 경유로 정합한다. 이력: 최초 v1.1 기준 작성 → v1.2(D-073 등급 제거·§3.3 응답 스키마) 정합 갱신(055 집행).
