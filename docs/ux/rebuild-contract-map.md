...
## 2.9 ★ 색·팔레트 원칙 (전 화면 적용 · 총괄 확정 2026-07-21)

**구조·레이아웃·간격·타이포 스케일 = 목업 그대로. 색 = 장터 브랜드 팔레트.**
- 목업은 Vuexy Bootstrap5 템플릿 위에 그려져 **기본 크롬 색(예: pill active `#3867df` 블루)이 브랜드가 아니다** — 템플릿 잔재다.
- 장터 팔레트(네이비 `#16213a` · 골드 · 오렌지 `#ef8a2c`, 메모리 `brand-identity`)가 사용자 자산 기반 정체성 앵커(비-AI 차별화). FC-067 토큰이 정본.
- 따라서 프론트는 **목업의 DOM·치수·레이아웃을 1:1로 따르되, 색상 값만 브랜드 토큰으로 치환**한다. 목업의 Vuexy 블루/그레이를 그대로 복제하지 않는다.
- "목업 그대로"(사용자 지시)는 **시각 구조**를 뜻하며, 색은 사용자가 이미 정한 장터 브랜드가 정본이다(폐기된 것은 우리 퍼플·레이아웃 결정이지 브랜드가 아니다).

---

## 3. 통화(코드) 표기 규칙

구 목업(§3.3)은 아이콘 `code.png`와 축약(`1만`/`248만`/`99억`)을 사용했으나 역사적 참고일 뿐이다. 현재 정본(v1.3)은 이미지 화폐 기호를 제거하고 숫자 뒤에 `코드`·`캐시` 텍스트 단위를 사용한다. 계약상 금액은 전부 **`long`(정수)**다.
...
| 맥락 | 표기 | 근거 |
|---|---|---|
| **마켓·경매 목록**(상단 가용잔액·`ItemCardView`·`AuctionCard`) | **정수 원본** (`2480000`→`2,480,000 코드`) | 사용자 최신 승인. 가격 정확성 우선 |
| **그 밖의 탐색면** | 필요 시 **축약** (`2480000`→`248만 코드`) | `compact` 호환 유지. `TopNavbar` 포함 |
...
- 이미지 화폐 기호와 `G`·`C`·`골드` 별칭은 제거한다. 숫자 뒤에는 `코드`·`캐시`를 표기한다.
- 축약은 **표시 변환일 뿐**, 상태·전송은 정수를 유지한다(왕복 변환으로 정밀도 잃지 말 것).

### 3.2 `<CodeAmount>` 컴포넌트 계약

```tsx
<CodeAmount value={number} currency="code" | "cash" mode="compact" | "full" />
```

- `value`: 정수(원본 `long`). 문자열 금지(정렬·비교·연산 정확성).
- `mode`:
  - `"compact"` → 숫자만 축약(`248만 코드`). `TopNavbar` 등 정확한 전체값이 주 시각 정보가 아닌 탐색면.
  - `"full"` → 천단위 구분(`2,480,000 코드`). 마켓·경매 목록 가격/가용잔액과 입찰·지갑·정산·상세·판매 입력.
- `currency`: 기본 `"code"`; 캐시 금액은 `"cash"`를 명시한다. 이미지 화폐 기호는 렌더하지 않는다.
- 접근성: `aria-label`은 **항상 full 전체값 + " 코드"/" 캐시"**(mode 무관).
...
3. **마감은 `now >= endAt` 클라 판정. 서버 status를 믿지 마라**(마감 강등 워커 부재, `resultType` 항상 null).
   ...
   소프트클로즈 재동기화.
4. **금액은 정수(long)로 다루고 축약은 표시 변환뿐.** `<CodeAmount value(정수) mode>` — API·거래확정과
   마켓·경매 목록엔 원본, 그 밖의 탐색면은 필요 시 축약하며 aria-label엔 항상 전체값. `minNextBidAmount`는 서버 파생(클라 증분표 복제 금지), 변할 때
