...
물 권역을 추가하면서 불 권역이 사라졌던 문제를 수정한 시리즈다. 모든 시안은 속성 4개와 중립 허브 1개를 각각 독립된 플레이 지역으로 구성한다.

- 공통 권역: 흙 고원·바람 군도·불 화산·물 산호도시·중립 마을 허브
- `world-map-five-regions-a.png`: 다섯 권역을 큰 랜드마크와 순환 다리로 연결한 균형형
- `world-map-five-regions-b.png`: 포털 환승 지점을 강조한 원형 순례 경로형
- `world-map-five-regions-c.png`: 거대 수호상·활화산·바람 정원·폭포 도시를 강조한 시네마틱형
...
- `world-map-game-sources-a-v2.png`
- 물 권역에 잘못 포함됐던 원형 기둥·룬 구조를 제거하고 좌하단 바람 숲의 기둥형 바람 성소로 이동
- 물 권역의 빈 자리에는 `map3/ground.png`를 참고해 선수·선미·선체·전체 갑판·돛대·밧줄·돛이 연결된 완전한 선박을 배치
- 불·흙·중립 권역과 중앙 콘텐츠 안전영역은 A 시안 구도를 유지
