...
8. **금액 표기(v1.3 정본).** 이미지 화폐 기호와 `G`·`C`·`골드` 별칭은 제거하고 숫자 뒤에 `코드`·`캐시`를 쓴다. 접근성 이름은 항상 전체값이며, 마켓·경매 목록의 상단 가용잔액과 실제 카드 가격은 축약 없이 정수 원본을 표시한다. 그 밖의 탐색면은 `compact`를 사용할 수 있다.
