...
상태: DRAFT v0 — 디자인 초안. 계약 v1.1 기준. 상세 응답 필드 스키마는 백엔드 구현 확정 시 정합.
소유: 디자인(UX/UI)
근거: api-contract v1.1(최상위 — 라우트·엔드포인트), domain-spec §3~§9, frontend/notes/screen-route-map.md(미확정 참고 — 프론트 노트), design-guide 5절
기준: 계약이 최상위. 계약에 없는 화면·데이터는 가정하지 않는다(공백은 6절에 결정 요청 대상으로 표기).
...
- 카드 표시: 아이템명·GradeBadge·ElementBadge·레벨/스킬 요약·현재 최고가(font-num)·Countdown·판매유형 칩(경매/즉시구매 여부).
...
- 와이어프레임: [좌·상] 아이템 스냅샷(이미지·명·Grade/Element·레벨·스킬1/2·발동확률·골드포스 잔여) · [우·상] 거래 패널(현재 최고가·최고입찰자 마스킹·Countdown·입찰 입력·입찰 버튼·즉시구매 버튼(설정 시)) · [하] 입찰 내역(offset 페이지, 마스킹) · 판매자 본인이면 취소 액션.
...
디자인이 화면을 그리며 발견한, 계약이 명시하지 않아 확정이 필요한 항목. 임의 확정하지 않고 결정 요청/확인 대상으로 남긴다(design-guide 1절).

1. 목록/상세 응답의 정확한 필드 스키마 — 계약은 "요약/스냅샷" 수준으로만 기술. 카드·상세에 표시할 필드 목록 확정은 백엔드 구현 응답 또는 기획 확인 필요(프론트 노트 §6과 동일 관찰). 착수 시 확인 대상으로 표기, 임의 확정 금지.
2. 아이템 등급(grade) 단계 수·명칭 — grade 색 매핑(design-system 2.3)의 전제. 기획 확인 대상.
3. 실시간 채널 — 푸시(SSE/WS) 부재로 폴링 전제(U-006, F-001 정합). 채널 도입 여부는 총괄 결정 사항(프론트 outbox/001 안건과 중복 — 이중 발제 대신 정보 공유로 정합만 확인).
4. 홈 통합 피드의 경매·고정가 혼합 표시 규칙(정렬·구분) — 계약에 통합 피드 스펙 없음. 탭 분리로 잠정 설계, 통합 노출 규칙 필요 시 결정 요청.
