...
상태: DRAFT v0.2 — IA 기준선. 계약 v1.4 기준. (053 총괄 회신) PF-001로 IA 기준선은 선채택됐으나, 이 문서는 라우트·엔드포인트·데이터 매핑의 정보구조 기준선이지 비주얼 확정본이 아니다. 화면 비주얼·핵심 UX 최종은 총괄+사용자(D-072).
소유: 기획 (P) — D-079로 PF 흡수·이관(구 소유 PF, PF-xxx 동결)
근거: api-contract v1.3(최상위 — 라우트·엔드포인트·응답 스키마), domain-spec v0.4(도메인 규칙), D-073(등급 축 제거), D-077(PF 신설)·D-079(PF→P 재통합), 053(총괄 회신 — 안건 1·2 (a) 채택), P-009~011(auth 폼·플로우)
...
- 엣지 오류(계약 §1.6, v1.3): auth 계열은 rate limit 대상 → `GATEWAY_429` 수신 시 `Retry-After` 존중 백오프. 기존 envelope 소비 로직 재사용(신규 파서 불요).
