...
> **작성 기준**: 2026-08-13 · 로컬 HEAD=`a837eb4` · 프론트 공통 UI 시스템과 후속 카드·모바일 회귀 수정 완료 · push 대기.
...
> **2026-08-13 최신 델타**: `EPIC-FRONTEND-UI-SYSTEM`(FC-270~282, Jira KAN-303~316)은 게이트3 승인으로 완료됐다. navy/gold/orange 토큰, 고정 AppShell chrome, route accent 격리, `ListFrame`, 카드 composition을 적용했고 카드 클릭·이미지 clipping·모바일 배경·CTA 대비 회귀까지 수정했다. 상세 상태와 다음 세션 순서는 `docs/board/HANDOVER.md`가 정본이다.
