...
성격: 참조 자료(유형 2). 프론트 Claude Code 세션 시작 시 이 파일 내용을 붙여넣거나 경로로 지정한다.
관련: D-069(핸드오프 규약), D-088(반환 경로), D-061(커밋 사용자 전담), D-087(대괄호 표기), 062(repo 후생성)
갱신: 작업 프롬프트 경로만 교체해 재사용한다. **현재 유효한 작업 프롬프트가 있는지는 `notes/handover.md`가 말한다 — 이 파일에 상태를 적지 않는다.**
...
작업을 마치면 **`D:\Java\finalcall\docs\frontend\notes\cc-reports\<NNN>-<도메인>-<작업>.md`** 에 보고를 남긴다(`<NNN>` = 작업 프롬프트 번호와 일치). 채팅 요약은 선택이다.

보고에 포함할 것:
- **결과**: 1~2줄.
- **산출물**: 생성 파일 트리 요지(전체 나열 말고 구조 수준).
- **DoD 대비**: 이번 작업 지시의 `[DoD]` 각 항목 충족/미충족 + 미충족 사유. 검증은 실측으로 — `lint`·`typecheck`·`build` 실행 결과를 적어라(추정 금지).
- **판단 보류·공백**: 계약·기획 문서에서 발견한 공백, 추측을 피해 멈춘 지점, 상위 문서 간 충돌.
- **커밋 메시지 제안**: 프론트 `rules.md` [6] 형식.
