...
갱신: 작업 프롬프트 번호만 교체해 재사용한다.

**현재 유효한 작업 프롬프트 없음 (2026-07-15).** 스켈레톤(006)은 완주했다(`notes/cc-reports/006-skeleton-scaffold.md`). `outbox/007`은 **VOID** — 실행하면 완성된 스켈레톤을 처음부터 재생성한다. 아래 "이번 작업"의 007 경로를 **그대로 붙여넣지 마라.** 다음 지시(A안→U-020 토큰 교체)는 디자인 회신(outbox/009) 후 발행하며, 그때 번호를 여기에 갱신한다.
