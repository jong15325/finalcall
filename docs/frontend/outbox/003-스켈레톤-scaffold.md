상태: HOLD — 보류(2026-07-14). 사유: 스켈레톤 기획 = 기획(P) 담당(D-079로 PF 재통합, 지시 063 인계). 레인 순서·repo 후생성은 총괄 확정(062, (a)). 상태: SUPERSEDED → frontend/outbox/006-스켈레톤-scaffold-재발신.md (2026-07-15). 보류 사유(계약 버전) 해소(028 — v1.4 통일), 반영 항목이 본문 다수를 바꾸므로 발신 이력 불변 원칙(D-023)에 따라 본 파일은 이력으로 보존하고 006으로 재발신한다.
--- 이하 보류 당시 기록 ---
