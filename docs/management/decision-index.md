...
| G4-n 도메인 구현 | 구현 DoD + QA 시나리오 + defects 처리, 전체 완료 시 보안 게이트 2 | 진행: auth·SCG 게이트웨이 구현 DoD 충족(검수 019·021). QA 기동(058)으로 auth+게이트웨이 검증 → G4-1 게이트 판정 중. 프론트 스켈레톤·디자인 초안 병렬. 백엔드(B) 대기 | - |
