...
| **원본 게임 UI 자산(또는 색상값) 미수령** — U-016 확정됐으나 디자인 세션에 원본이 없어 픽셀 샘플링 불가, 팔레트 hex가 육안 추정 잠정값. **토큰명·구조는 고정 가능해 프론트 스켈레톤은 미차단**(값만 교체) | 블로커 | 사용자·디자인 | 사용자가 원본 UI 자산 또는 색상값 제공 → design-system [2] 색 토큰 게임 팔레트로 재정의 | OPEN |
| 상거래 카드·판매등록 레인 블로킹 — **원인 변경**: 게임 이미지 미제공(U-012) → **스펙 공백**(계약 [3.3] item 블록에 이미지 참조 필드 부재, domain-spec [7]이 시각 자산 관리를 범위 밖에 둠). 이미지 슬롯의 데이터 출처가 스펙에 없어 디자인이 임의 가정 불가 | 블로커 | 기획·디자인 | 디자인 ux/outbox/010·012 결정 요청 → **기획 회신 대기**. auth·탈퇴 등 게임 외 웹 페이지는 무관하게 진행 중 | OPEN |
