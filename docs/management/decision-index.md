...
| G3 api-contract 확정 | 총괄 검수 + 보안 게이트 1 + 사용자 승인 → 백/프론트/QA/보안 기동(백엔드는 G1 직후 기술 규약 협의 모드로 조기 기동, D-035) | 총괄 검수 통과 · 보안 게이트 1 조건부 통과(S-001) · 계약 반영 중(SEC 7건) · 사용자 승인 대기 | - |
...
| ERD G2 관찰 후속: sale_order 폴리모픽 FK(B-001 정합)·shop(item_instance_id) 인덱스 | 리스크 | 기획·백엔드 | 백엔드 정보공유 확인·인덱스 검토 | OPEN |
