...
| 컨설턴트 종료 재검토 (D-042 대기 모드) | 보류 | 총괄·사용자 | **2026-07-15 기동**(077 파생 순서 체계 회부, 안건5에 존치 여부 권고 포함) | 기동됨 |
| 파생 작업으로 인한 순서 붕괴 — 착수 근거 미검증(066→022)·파생 선행범위 총괄 임의확대로 진행중 작업 충돌(073→076)·같은 뿌리 파생 층간 재발(022/028) | 리스크 | 총괄·컨설턴트 | 077 회부(안건 1~4: 착수 전 근거검증·진행중 작업 보호·파생범위 판정권한·연쇄 가시화) → 컨설턴트 권고 → 총괄 D 발번·전파 | OPEN |
