...
| 검증 도구의 구조적 사각지대 3건 — QA 검증기준 erd 누락(028)·bash 마운트 stale 뷰(qa/002·§5 유령절단 오탐)·import 기준 grep의 동일 패키지 미포착(031). 공통 형태 = "도구가 못 보는 영역이 있는데 그걸 모르고 신뢰" | 리스크 | 총괄·컨설턴트 | 각 건 국소 조치 완료(075 기준 보완·D-043 교차검증·백엔드 자체 재발방지). **규약화 필요성 판단 078로 회부** — 총괄 심증=국소 조치로 충분(도구마다 사각지대가 달라 일반 규칙화 시 공허) | OPEN |
