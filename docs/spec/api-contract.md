...
- **비보장(advisory)**: `available:true`는 조회 시점 스냅샷일 뿐 **예약이 아니다**. 조회~제출 사이 선점(TOCTOU)이 가능하므로 최종 권위는 signup의 `AUTH_002`(409)·`PATCH /me`의 `MEMBER_001`(409)이다. 프론트는 `available:true`여도 제출 시 409를 처리한다(§4 프론트 매핑 재사용).
...
- **비보장(advisory)**: `available:true`는 조회 시점 스냅샷일 뿐 **예약이 아니다**. 조회~제출 사이 선점(TOCTOU)이 가능하므로 최종 권위는 signup 의 `AUTH_001`(409, "이미 사용 중인 로그인 아이디입니다.")다. 프론트는 `available:true`여도 제출 시 409(`AUTH_001`)를 처리한다.
- 에러: 형식·길이 위반 시 **400**(COMMON 검증, `errors[]` 필드 매핑). **신규 도메인 에러코드 없음**.
