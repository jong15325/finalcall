...
완료 보고(작업 종료 시):
- **출력 방법 — 반드시 지켜라. 완료 보고는 파일로 저장하지 말고 채팅 응답 본문에 그대로 출력한다.**
  - 저장 위치를 사용자에게 묻지 마라. 저장할 곳은 없다.
  - `docs/backend/outbox/`·`docs/` 어디에도 완료 보고 파일을 만들지 마라. outbox 는 백엔드 대화의 소유이고
    Claude Code 는 파일 버스 노드가 아니다(D-069) — 네가 쓰면 발신 이력이 오염된다.
  - 사용자가 네 채팅 출력을 복사해 백엔드 대화로 중계한다. **그게 유일한 반환 경로다.**
  - 네가 만드는 파일은 코드·테스트·마이그레이션뿐이다. 보고·문서는 채팅으로만 낸다.
- 형식(채팅 출력): `# [Claude Code → 백엔드] 완료 보고: <도메인> - <작업>`
- 포함: 결과 요약, 산출물(파일 경로), 완료 기준 대비 충족 여부(빌드·테스트 결과), 이슈(백엔드 흡수 요망), 다음 단계 제안, 회신 필요/불요, 신규 발번 ID(없으면 없음).
- 별도 outbox·수신함 스캔 없음(D-069).
- 작업 중 질문·이슈도 같다 — 파일이 아니라 채팅으로 낸다. 백엔드가 흡수해 버스로 재진입시킨다.
