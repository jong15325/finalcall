...
## B-029. 잔액 행 부재 처리 — 불변식 위반(COMMON_999) · MEMBER_001 잠정 사용 철회 (2026-07-15) [ACCEPTED]

- 소유: 백엔드 / 관련: relates-to 024(잔액 조회), 071(총괄 지적 "응답에 실리면 계약 대상"), 070(기획 MEMBER_ 채번 조율), D-028. supersedes 026 사전 통지분(MEMBER_001 잠정 사용)
- 결정: `GET /api/v1/me/balance`에서 잔액 행이 없는 경우를 `MemberErrorCode.MEMBER_001`(404) 대신
  `CommonErrorCode.INTERNAL_ERROR`(COMMON_999, 500)로 처리한다. 024 유닛에서 `MemberErrorCode`를 신설하지
  않는다. `MEMBER_001` 잠정 점유를 철회하여 기획의 §5 채번을 무제약으로 해제한다(070 통지 대상).
- 이유: signup이 `user`와 `user_balance`를 1:1로 함께 생성하므로(V3 스키마 UK+FK, `UserBalanceRepository`
  주석 명시) 잔액 행 부재는 **비즈니스 상태가 아니라 깨진 불변식**이다. 404 `MEMBER_001`로 두면 "잔액 없는
  회원"이 정상 도메인 상태인 것처럼 계약 §5에 드러나는데 설계상 그런 상태는 존재하지 않는다. 500은
  "일어나선 안 되는 일이 일어났다"는 사실을 정확히 표현하고, 클라이언트가 복구할 수 없다는 점과도 맞다.
  부수 효과로 계약 표면이 사라져 071의 "번호 고정 의존 금지" 요구를 구조적으로 충족한다(상수 격리보다 근본적).
- 기각된 대안: `MEMBER_001` 404 유지(존재하지 않는 도메인 상태를 계약에 노출 + 기획 채번 제약),
  상수 참조로 격리 후 계약 확정 시 교체(071 권장이나, 애초에 계약 대상이 아니면 격리 자체가 불요 — 문제를
  옮길 뿐 없애지 못함).
- 후속: `MemberErrorCode`는 프로필·수정·탈퇴 유닛에서 계약 §5 확정 번호(기획 069)로 신설한다. 이 철회를
