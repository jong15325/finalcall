...
## 하지 말 것
- `PATCH /me`(수정)·`DELETE /me`(탈퇴) — **별도 유닛이다. 이 유닛에서 착수 금지.**
  주(갱신): 종전 금지 사유였던 재가입 UK 선행(028)은 **D-081·V4(030)로 해소됐다.** 이제 순수 유닛 분리
  사유다 — 각각 계약 [2.5]의 독립 조항이고 커밋을 나눈다. `MemberController`에 미리 넣지 마라.
...
범위 밖(다음 단계)
- PATCH /me·DELETE /me(별도 유닛 — V4 선행 해소됨), 타인 프로필 조회(계약 범위 밖)
```
