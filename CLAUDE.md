...
- **비고**: 위 도메인은 스켈레톤(Stage 0~G)이 완성된 뒤 구현한다. 스켈레톤 단계에서는
  notice(공지사항)를 참조 구현으로 만들어 컨벤션의 본보기로 삼았으나, EPIC-BOARD(FC-201)에서
  notice는 게시판 시스템의 '공지' 게시판으로 흡수·제거됐다 — 이제 **board(게시판) 도메인이 참조 구현을
  승계**한다(커서 목록·soft delete·IDOR 차단·다중 엔티티·2단계 이미지 업로드·비정규화 카운터로 더 풍부).
...
- **의존 방향(feature-first, EPIC-RESTRUCTURE)**: 세 축을 ArchUnit(`LayerDependencyTest`)이 기계 강제한다.
  ...
  (3) **슬라이스 비순환** — feature 간 순환(top-level) 참조 금지.
  규칙 스펙 = proposal §10. 신 규칙 3종(슬라이스 계층방향·커널 격리·top-level 비순환)만 남는다 — 구 최상위-레이어 규칙(`api→domain→infra→common`)은 재구성 완료로 제거됐다.
...
- **파일 → Jira 단방향**. Jira Cloud(KAN)는 **사용자 전용 읽기 미러**이며, 전송 수단은 **Jira Cloud REST API**다. 메인세션만 미러·패리티 대조를 위해 Jira를 읽고 쓰며, 서브에이전트는 Jira에 접근하지 않는다.
